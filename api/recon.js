export default async function handler(req, res) {
  ...
  // Accept bottlenecks array OR singular bottleneck (back-compat)
  let bottlenecks = [];
  if (Array.isArray(body.bottlenecks)) {
    bottlenecks = body.bottlenecks.filter(Boolean);
  } else if (body.bottleneck) {
    bottlenecks = [body.bottleneck];
  }

  if (!category || !brand || bottlenecks.length === 0) {
  ...
  lines.push("Bottlenecks (1-3): " + bottlenecks.join(", "));
  if (bottlenecks.length > 1) {
    lines.push("Spread the 3 briefs across the bottlenecks above.");
    lines.push("Each brief should clearly address one of them.");
  }
  lines.push("");
  ...
  lines.push("- Be specific. Avoid AI jargon.");
  lines.push("");
